...
unauthorized attribution trailers in commit messages. The primary
target is `Co-authored-by:` — the most common agent auto-insertion.
Other trailers (`Signed-off-by:`, `Reviewed-by:`, `Tested-by:`,
`Suggested-by:`) are legitimate open-source practices and allowed
by default.
...
> **MUST NOT include `Co-authored-by:` or other unauthorized `*-by:`**
> **attribution trailers in commit messages.** Legitimate open-source
> trailers (`Signed-off-by:`, `Reviewed-by:`, `Tested-by:`,
> `Suggested-by:`) are allowed by default and do not need a bypass.
> Use `ALLOW_ATTRIBUTION_TRAILERS=1` to allow all trailers when
> a genuine cross-user commit requires it.
...
> ### Commit trailers
>
> This project blocks unauthorized attribution trailers in commit
> messages. The primary concern is `Co-authored-by:`, which agents
> may auto-insert. Legitimate open-source trailers (`Signed-off-by:`,
> `Reviewed-by:`, `Tested-by:`, `Suggested-by:`) are allowed by default.
> If you have a genuine need for other cross-user attribution, set
> `ALLOW_ATTRIBUTION_TRAILERS=1` in your environment and document why
> in the commit body.
...
1. **Pair-programming teams.** If every commit genuinely involves co-authors,
   `Co-authored-by:` is legitimate. Set `ALLOW_ATTRIBUTION_TRAILERS=1` for
   the repo's CI and skip the hook installation. Document the decision in
   `.repo-health.json`.

2. **DCO-enforced projects.** Projects that require `Signed-off-by:` (Linux
   kernel, CNCF, many foundations) are already handled — this trailer is
   allowed by default. No configuration change needed.

3. **Mailing-list workflows.** Projects accepting patches via email or
   Gerrit (`Reviewed-by:`) are already handled — these trailers are allowed
   by default.

4. **Single-contributor repos.** One person working alone is unlikely to
