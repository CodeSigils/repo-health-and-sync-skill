...
REQUIRED_PATTERNS = [
    (r"def (?:do_self_test|run_self_tests|check_self_test)\(\)", "missing --self-test function"),
...
# Shell scripts don't need Python-specific patterns
...
def check_script(script_path: Path) -> list[str]:
    ...
    if script_path.suffix == ".sh":
        ...
        # Check for set -euo pipefail (recommended)
    ...
    for call in open_calls:
        if "encoding=" not in call and "rb" not in call and "wb" not in call and "urllib.request.urlopen" not in call:
