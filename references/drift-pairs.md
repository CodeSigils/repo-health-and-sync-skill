...
Severity depends on commit quality (see B7b in SKILL.md).

```bash
baseline="${1:-origin/main}"
if [ -f CHANGELOG.md ]; then
  # CHANGELOG exists — enforce strict drift check
  ...
  # No CHANGELOG — assess commit quality instead (run B7a-style check)
  echo "INFO: no CHANGELOG.md found — commit log quality determines severity"
  # The agent should sample last 10 commits (see B7b in SKILL.md for full logic)
fi
```
...
Verify that `## Unreleased` exists when there are commits since the last
tag. Only checks when CHANGELOG.md exists — projects without one rely on
commit quality assessment from B7b.

```bash
if [ ! -f CHANGELOG.md ]; then
  echo "INFO: no CHANGELOG.md — commit log serves as release record"
  exit 0
fi
...
inventory and actual scripts are in sync.
